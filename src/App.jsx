...
export default function App() {
  ...
  const aiStatus = useMemo(() => {
    ...
    if (lastProvider) {
      const isGroq = lastProvider.toLowerCase().includes("groq");
      const isOr = lastProvider.toLowerCase().includes("openrouter");
      const isGemini = lastProvider.toLowerCase().includes("gemini");
      const tag = isGroq ? "GROQ (FAST)" : isOr ? "OPENROUTER" : isGemini ? "GEMINI" : "AI READY";
      return {
        label: `AI READY (${tag})`,
        className: "ready",
        title: `Study session powered by ${lastProvider}`
      };
    }
    return {
      label: "AI READY",
  ...
  const generateStudySession = useCallback(
    async (customTopic = topic) => {
      ...
      try {
        let response;
        try {
          response = await fetch(`${apiBase}/api/study-session`, {
            method: "POST",
          ...
          // If direct apiBase failed with network error and apiBase was non-empty, try relative /api proxy
          if (apiBase && !controller.signal.aborted) {
            try {
              response = await fetch("/api/study-session", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: reqPayload,
                signal: controller.signal
              ...
              throw fetchErr; // Throw original network error
            }
          } else {
            throw fetchErr;
          }
        ...
        if (!response.ok) {
          if (response.status === 401 || response.status === 403 || body?.error?.toLowerCase().includes("api key")) {
            throw new Error(body?.error || "AI provider API key error or expired key. Please check or provide a new API key.");
          }
          if (response.status === 504 || body?.error?.includes("too long to respond")) {
            throw new Error(body?.error || "The AI server is taking longer than expected. Please click Retry.");
          }
        ...
        if (wasTimeout) {
          setError("The request took longer than 35 seconds. Please click Retry.");
          setPhase("error");
          return;
        }
        ...
        let msg = err?.message || "Unable to generate study session. Please click Retry.";
        if (msg.includes("Failed to fetch") || msg.includes("NetworkError")) {
          msg = "MindForge server could not be reached (Failed to fetch). Please ensure the backend is running on port 3001 and click Retry.";
        }
                  ...
                  <div className="session-title-block">
                    <div className="session-tag-row">
                      ...
                      {lastProvider === "cache" ? (
                        ...
                        <span className="cache-indicator" title={`Generated by ${lastProvider}`}>
                          🤖 {lastProvider.toLowerCase().includes("groq") ? "Groq (Fast)" : lastProvider.toLowerCase().includes("openrouter") ? "OpenRouter" : "Gemini"}
                        </span>
                      ) : null}
                    </div>
                    <h2>{session.topic}</h2>
